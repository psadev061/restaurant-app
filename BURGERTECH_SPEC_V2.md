...
| `subtotal_bs_cents` | `integer` | NOT NULL | Convertido con la tasa del día |
| `dynamic_cents_surcharge` | `integer` | NOT NULL, CHECK 1–99 | Céntimos únicos para conciliación |
| `exact_amount_bs_cents` | `integer` | NOT NULL | `subtotal_bs_cents + dynamic_cents_surcharge` |
...
| `table_id` | `uuid` | — | FK → `tables.id` cuando `order_mode = 'dine_in'` — **v2 preparado** |
| `delivery_address` | `text` | — | Dirección de texto libre cuando `order_mode = 'delivery'` — **v2 preparado** |
| `delivery_address_refs` | `text` | — | Referencias del punto de entrega — **v2 preparado** |
| `delivery_coords` | `jsonb` | — | `{lat, lng}` GPS opcional — **v2 preparado** |
| `delivery_zone_id` | `uuid` | — | FK → `delivery_zones.id` — **v2 preparado** |
| `delivery_cost_usd_cents` | `integer` | NOT NULL, default `0` | Costo de delivery en USD cents — **v2 preparado** |
| `created_at` | `timestamptz` | NOT NULL, default `now()` | |
| `updated_at` | `timestamptz` | NOT NULL, default `now()` | |
...
ON orders (exact_amount_bs_cents)
...
|---|---|---|---|
| `id` | `uuid` | PK | |
| `amount_bs_cents` | `integer` | NOT NULL | Monto recibido × 100 |
...
| `whatsapp_number` | `text` | NOT NULL | Número para el modo WhatsApp. Ej. `584141234567` |
